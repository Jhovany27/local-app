...
<body class="bg-[#edf3e3] min-h-screen flex items-center justify-center p-4">
    ...
    {{-- BOTÓN REGRESAR A SELECCIONAR TIENDA (solo si tiene rol tienda y al menos una tienda activa) --}}
    @auth
        @php
            $user = auth()->user();
        @endphp
        @if ($user->hasRol('tienda') && $user->tiendas()->where('tie_estado', \App\Models\Tienda::ESTADO_APROBADA)->exists())
            <a href="{{ route('filament.store.pages.seleccionar-tienda') }}"
        ...
        @endif
    @else
        {{-- BOTÓN PORTAL (usuario no autenticado) --}}
        <a href="{{ route('portal') }}"
            class="fixed top-5 left-5 inline-flex items-center gap-2 bg-white/80 backdrop-blur-sm
              text-gray-600 text-sm font-semibold px-4 py-2 rounded-full shadow-sm
              border border-gray-200 hover:bg-white hover:text-gray-900 transition-all z-50">
        ...
        <div class="p-6 md:p-12 flex flex-col justify-center">
            ...
            <div class="space-y-4">
                @foreach ($tiendas as $tienda)
                    {{-- CARD TIENDA --}}
                    <div
                        class="border-2 rounded-xl p-4 {{ $tienda->tie_estado == 0 ? 'border-amber-300 bg-amber-50' : 'border-red-300 bg-red-50' }}">
                        ...
                        <div class="flex items-center justify-between mb-3">
                            ...
                            @if ($tienda->tie_estado == 0)
                                <span
                                    class="inline-flex items-center gap-1 bg-amber-100 text-amber-700 border border-amber-300 text-xs font-bold px-2.5 py-1 rounded-full">
                                    <span class="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse"></span>
                                    Pendiente
                                </span>
                            @else
                                <span
                                    class="inline-flex items-center gap-1 bg-red-100 text-red-700 border border-red-300 text-xs font-bold px-2.5 py-1 rounded-full">
                        ...
                        @if ($tienda->tie_estado == 0)
                            ...
                            <div class="space-y-2">
                                <div class="flex items-center gap-2 text-xs text-gray-600">
                                    <div
                                        class="w-5 h-5 rounded-full bg-[#a8df11] flex items-center justify-center flex-shrink-0">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke-width="2.5" stroke="white" class="w-3 h-3">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <div class="flex items-center gap-2 text-xs text-amber-700 font-semibold">
                                    <div
                                        class="w-5 h-5 rounded-full bg-amber-100 border-2 border-amber-400 flex items-center justify-center flex-shrink-0">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke-width="1.5" stroke="#f59e0b" class="w-3 h-3">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        @else
                            {{-- RECHAZADA --}}
                            <div class="mb-3">
                                <p class="text-xs font-bold text-red-600 uppercase tracking-wide mb-1">Motivo del
                                    rechazo</p>
                                <p
                                    class="text-sm text-gray-700 bg-white border border-red-200 rounded-lg px-3 py-2 leading-relaxed">
                                    {{ $tienda->tie_motivo_rechazo ?? 'No se especificó un motivo.' }}
                                </p>
                            </div>
                            <a href="{{ route('store.editar-tienda') }}?id={{ $tienda->tie_id }}"
                                class="block w-full text-center bg-[#a8df11] hover:bg-[#95c510] text-gray-900 font-bold py-2 rounded-lg transition text-sm">
